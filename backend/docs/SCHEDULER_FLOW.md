...
| ตารางข้อมูลเครื่องจักร + จิ๊กเสริม | ตัวเลือกเครื่อง เวลาต่อชิ้น **เวลาหยิบจับ** เวลาตั้งเครื่อง ชุดจิ๊ก | กำหนดว่าขั้นตอนนี้แข่งกันกี่เครื่อง และกินเวลาเท่าไหร่ |
...
| คอลัมน์ "ล็อกเส้นทาง" (`flow_locked`) ในตารางใบสั่ง | เลือกเส้นทางเองที่ขั้นตอนแรกไม่มีผล — เส้นทาง 0 ไม่ล็อก เส้นทางอื่นกลับไปเป็นแบบ WIP เดิม (หัวข้อ 7ข, 9) |
...
> **"มีประวัติเส้นทางค้างอยู่" = `wip_flow_index > 0`** (ของเดิมจาก Python) **ยกเว้นใบที่ผู้ใช้เลือกเส้นทางเอง**
> (`orders.flow_locked = 1` และเริ่มที่ขั้นตอนแรก = manual flow) — ใบแบบนี้ยังไม่ได้ผลิต จึงไม่ติดป้ายคิวฉุกเฉิน
> และไปสายตามที่ผู้ใช้ตั้ง (เดินหน้าหรือถอยหลัง) โดยใช้แค่เส้นทางที่ล็อกไว้ ส่วนใบที่ไม่ล็อกยังเป็นแบบเดิมทุกอย่าง
...
flowchart TD
    ...
    SCAN --> BOOK["จองเวลาผลิตก่อน จนกว่าจะครบจำนวนงานค้าง<br/>วันที่เหลือเวลาว่างน้อยกว่า 120 นาที ถือว่าใช้ไม่ได้<br/>จำนวนที่ทำได้ต่อวัน = เวลาว่าง หารด้วย เวลาต่อชิ้นจริง ปัดลง<br/>เวลาต่อชิ้นจริง = เวลาต่อชิ้น บวก เวลาหยิบจับ"]
...
flowchart TD
    ...
    C3 -->|"เคยผลิตแล้ว"| LOCK2["ใช้เส้นทางที่ตรงกับของที่ผลิตไปแล้วมากที่สุด<br/>(ให้คะแนน 4 ชั้น ดูตารางข้างล่าง)"]
...
**ใบที่เคยผลิตแล้วเลือกเส้นทางยังไง** (`scheduler/flowPick.js` → `pickFlowByActuals`) ให้คะแนนทีละชั้น เสมอกันค่อยดูชั้นถัดไป

1. จำนวนชื่อขั้นตอนที่ผลิตไปแล้วซึ่งอยู่ในเส้นทางนั้น (มากสุดชนะ)
2. จำนวนขั้นตอนที่เครื่องที่บันทึกยอดจริงเป็นหนึ่งในเครื่องทางเลือกของเส้นทางนั้น
3. จำนวนชื่อขั้นตอนของแผนเดิม (`schedule_results`) ของใบนี้ที่อยู่ในเส้นทางนั้น
4. เส้นทางที่มาก่อนชนะ

> ⚠️ **ของเดิม (port จาก Python ตรงตัว) หยิบเส้นทางแรกที่มีชื่อขั้นตอนตรงแค่ตัวเดียว** — ขั้นตอนที่มีในทุกเส้นทาง
> เช่น HEAT-TREATMENT จึงลากใบไปเส้นทางแรกเสมอ แล้ววางขั้นตอนที่ผลิตเสร็จไปแล้วซ้ำ (เจอจริง: 1ST-2ND เสร็จแล้ว
> แต่แผนใหม่วาง 1ST + 2ND อีกรอบ) หรือย้ายงานที่ทำไปครึ่งทางไปเส้นทาง OUTSOURCE — แก้เป็นกติกาข้างบนแล้ว
> ไม่มี parity fixture ไหนมียอดจริงบนรุ่นหลายเส้นทาง การแก้นี้จึงไม่กระทบ parity

> **เส้นทางที่ผู้ใช้เลือกเอง (`orders.flow_locked = 1`) = "ถูกบังคับมา" ในเงื่อนไขที่ 4 รวมเส้นทาง 0 ด้วย** — ไม่จำลอง
> และไม่เลือกตามยอดจริง ก่อนมีคอลัมน์นี้ เส้นทาง 0 ที่เริ่มขั้นตอนแรกบันทึกเป็น 0/0 ซึ่งแยกไม่ออกจาก "ไม่ได้เลือก"

> **WIP ที่ระบุขั้นตอน (`wip_start_step_index > 0`) = ผู้ใช้เลือกเส้นทางแล้ว แม้จะเป็นเส้นทาง 0** — ไม่ถูกยอดจริงเปลี่ยนทับ
> และไม่โดนบรรทัด "ถอยไปใช้เส้นทางแรก" ทับ (เดิมเลข 0 ถูกตีความว่า "ยังไม่เลือก" เสมอ)
> หน้า Tracking ของ order (`GET /orders/:batchId/tracking`) ใช้กติกาเดียวกันผ่าน `resolveTrackingFlow`
...
flowchart TD
    ...
    F -->|"ถึง"| F2["ใส่เวลาตั้งเครื่องให้ครบก่อน<br/>เวลาที่เหลือ หารด้วย เวลาต่อชิ้นจริง ปัดลง<br/>เวลาต่อชิ้นจริง = เวลาต่อชิ้น บวก เวลาหยิบจับ<br/>= จำนวนที่ผลิตได้ในวันนั้น"]
    JIG1 --> RE
    F1 --> RE
    F2 --> RE
...
> **ล็อกเครื่องได้สองทาง และทั้งสองทางตัดตัวเลือกเหลือเครื่องเดียว** (`narrowToMachine` ใน `engine.js`)
> 1. กฎล็อกเครื่องในแผนภาพข้างบน — ขั้นตอนที่ผลิตไปแล้วบางส่วน
> 2. **ใบที่ผู้ใช้เลือกเส้นทางเองและเลือกเครื่องของขั้นตอนแรกไว้** (`flow_locked = 1`, เริ่มขั้นตอนแรก, `wip_machine`)
>    ล็อกเฉพาะขั้นตอนแรก ขั้นตอนถัดไปเลือกตามปกติ ใช้ทั้งสายเดินหน้าและสายถอยหลัง
>    ถ้าเครื่องนั้นไม่อยู่ในตัวเลือกของขั้นตอนแล้ว (ถูกปิด/routing เปลี่ยน) จะไม่ล็อก ให้ระบบเลือกเองแทนการตกแผน
>
> ⚠️ เวลาต่อชิ้น/เวลาตั้งเครื่อง/จิ๊ก ต้องมาจาก **เครื่องที่ล็อก** — ของเดิมตัดเหลือแค่รายชื่อเครื่อง แต่ยังจับคู่เวลาด้วยตำแหน่งแรก
> เครื่องที่ล็อกซึ่งไม่ใช่ทางเลือกลำดับแรกจึงได้เวลาของเครื่องอื่น แก้แล้ว (ผลเท่าเดิมเป๊ะเมื่อเครื่องที่ล็อกเป็นลำดับแรก)
> เมื่อเหลือเครื่องเดียว กติกาติดเครื่องเดิมไม่มีอะไรให้สลับ

### กฎที่ต้องเข้าใจให้ลึก

**0. เวลาต่อชิ้นจริง = เวลาต่อชิ้น + เวลาหยิบจับ**

ช่อง "เวลาต่อชิ้น" กับ "เวลาหยิบจับ" กรอกแยกกันเพราะหน้างานวัดคนละที แต่ตอนคำนวณ
เครื่องคำนวณ**บวกสองค่านี้เข้าด้วยกัน**แล้วใช้เป็นเวลาต่อชิ้นจริงทุกที่ —
ทั้งตอนหาว่าวันหนึ่งทำได้กี่ชิ้น และตอนหักเวลาออกจากปฏิทิน

สามข้อที่ต้องแยกให้ออกจาก "เวลาตั้งเครื่อง":

1. เวลาหยิบจับคิด **ต่อชิ้น** ส่วนเวลาตั้งเครื่องคิด **ครั้งเดียวต่อล็อต**
2. เวลาตั้งเครื่องถูก**ตัดเป็นศูนย์**เมื่องานนั้นเดินไปแล้ว (งานค้างเครื่อง) แต่**เวลาหยิบจับไม่ถูกตัด**
   เพราะการหยิบชิ้นงานเข้าออกยังเสียเวลาอยู่ไม่ว่างานจะเริ่มไปแล้วหรือไม่
3. เวลาตั้งเครื่องมีส่วนลดเมื่อใช้ชุดจิ๊กเดิมต่อกัน (ดูหัวข้อ 10) **เวลาหยิบจับไม่มีส่วนลดใด ๆ**

⚠️ **ขั้นตอนกลุ่ม "คิดเป็นวัน" ไม่คิดเวลาหยิบจับเลย** — ดูข้อ 1 ข้างล่าง ช่องเวลาต่อชิ้นของกลุ่มนั้น
หมายถึงจำนวนวัน ไม่ใช่นาที การเอาเวลาหยิบจับไปบวกจึงเป็นการบวกวัน ซึ่งผิดความหมาย
กรอกเวลาหยิบจับให้แถวส่งออกนอก/อบชุบไว้ก็ไม่มีผลอะไร (ไม่ใช่บั๊ก)

ไม่กรอก หรือฐานข้อมูลยังไม่มีคอลัมน์นี้ = ศูนย์ = ผลลัพธ์เท่ากับก่อนมีฟีเจอร์นี้ทุกประการ
...
| `scheduler/flowPick.js` | เลือกเส้นทางของใบที่ผลิตไปแล้ว (engine) + เส้นทางที่หน้า Tracking แสดง |
...
| 9 ใบที่ผลิตไปแล้วเลือกเส้นทาง | `scheduler/flowPick.js` | `pickFlowByActuals` (หน้า Tracking: `resolveTrackingFlow`) |
